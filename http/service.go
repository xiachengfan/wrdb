...
type Store interface {
	...
	Get(key string, lvl wrdbserver.ConsistencyLevel) (string, error)
...
func (s *Service) handleJoin(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.store.Join(nodeID, httpAddr, raftAddr); err != nil {
		if err == wrdbserver.ErrNotLeader {
...
func level(req *http.Request) (wrdbserver.ConsistencyLevel, error) {
	q := req.URL.Query()
	lvl := strings.TrimSpace(q.Get("level"))

	switch strings.ToLower(lvl) {
	case "default":
		return wrdbserver.Default, nil
	case "stale":
		return wrdbserver.Stale, nil
	case "consistent":
		return wrdbserver.Consistent, nil
	default:
		return wrdbserver.Default, nil
	}
}
